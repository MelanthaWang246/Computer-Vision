...
def sobelY(img):
    ...
    # Otherwise some pixel will become white becasue of data loss
    ...
    for r in range(row-2): # [0, row-3]
        for c in range(column-2): #[0, column-3]
            ...
            if result_xy[r+1, c+1] >= max:
                # get the maximum value for rearranging
...
if __name__ == '__main__': 
    '''read the image and switch to gray image
       for more image details resaved in processing'''
    path = r"D:\Finalpython\AS3\Lenna.png"
    img = cv2.imread(path, flags=0) 

    sobelx = sobelX(img)
    sobely = sobelY(img)
    # magnitude of x,y gradients
    sobelxy = sobelXY(img, sobelx, sobely)

    # set the threshould value
    thValue1 = 110
    thValue3 = 130
    thValue5 = 150
    binary110 = binaryImg(thValue1, sobelxy)
    binary130 = binaryImg(thValue3, sobelxy)
    binary150 = binaryImg(thValue5, sobelxy)

    cv2.imshow('binary110', binary110)
    cv2.imshow('binary130', binary130)
    cv2.imshow('binary150', binary150)
    cv2.waitKey(0)
